...
TCPInterface::TCPInterface (TCPInterfaceMode mode, int port) : mServerPort(port),
                                                               mMode(mode)
{
    if (mode == TCPCLIENT)
        setupClient();
...
void TCPInterface::setupClient (void)
{
	...
	mServerAddress.sin_family = AF_INET;
	mServerAddress.sin_port   = htons(mServerPort);  // host to network short: converts a u_short from host to TCP network byte order (big-endian).
	retVal = inet_aton(SERVER_ADDRESS, &(mServerAddress.sin_addr));
...
void TCPInterface::setBlocking (bool_t blocking)
{
    ...
    if (blocking)
        fcntl(mClientSocket, F_SETFL, !O_NONBLOCK); // TODO is this the right socket?
    else
...
void TCPInterface::checkForClients (void)
{
    CHECK_TCPMODE(TCPSERVER);
    socklen_t sizeofmClientAddress = sizeof(mClientAddress);
    mClientSocket = accept(mServerSocket, (struct sockaddr*) &mClientAddress, &sizeofmClientAddress);
...
void TCPInterface::writeBytes (const void* bs, size_t n)
{
    int retVal;
    if (mMode == TCPCLIENT)
        retVal = write(mClientSocket, bs, n);
    else
        retVal = write(mServerSocket, bs, n);
    CHECK_RETURN(retVal, "writeBytes");
}


/// @brief  Reads the given number of bytes from the socket.
...
void TCPInterface::readBytes (void* bs, size_t n)
{
    int retVal;
    if (mMode == TCPCLIENT)
        retVal = read(mClientSocket, bs, n);
    else
        retVal = read(mClientSocket, bs, n);    // TODO is this right?
    CHECK_RETURN(retVal, "readBytes");
}
